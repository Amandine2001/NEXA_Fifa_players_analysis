...
def get_filters(df):
    ...
    gender = st.sidebar.selectbox(
        "Genre", ["Tous"] + sorted(df["gender"].dropna().unique())
    )

    league = st.sidebar.selectbox(
        "Championnat", ["Tous"] + sorted(df["League"].dropna().unique())
    )

    position = st.sidebar.selectbox(
        "Poste", ["Tous"] + sorted(df["Position"].dropna().unique())
    )

    nation = st.sidebar.selectbox(
        "Nationalité", ["Toutes"] + sorted(df["Nation"].dropna().unique())
    )
